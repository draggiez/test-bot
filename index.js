...
async function runBot () {
    ...
    sock.sendImageAsSticker = async (jid, path, quoted, options = {}) => {
		...
		var convertapi = require('convertapi')('kB6rT6EDhEhIwru4',{
			conversionTimeout: 1200,
			uploadTimeout: 1200,
			downloadTimeout: 1200
		});
		await convertapi.convert('webp', {
			File: `${tmpFileIn}`,
			ScaleProportions: 'false',
			ImageQuality: '100'}, 'jpg')
			.then(function(result) {
				//console.log("Converted file url: " + result.file.url)
				console.log(`${chalk.bold.yellow("[+]")} ${chalk.bold.green("Converting to sticker...")}`)
				return result.file.save(`${tmpFileOut}`)
			})
			.then(function(file) {
				console.log(`${chalk.bold.yellow("[+]")} ${chalk.bold.green("Converted : " + file)}`)
				console.log(`${chalk.bold.yellow("[+]")} ${chalk.bold.green("Sticker sent.")}`)
				//console.log("File saved: " + file);
			})
			.catch(function(e) {
				console.log(`${chalk.bold.yellow("[+]")} ${chalk.bold.red("Error converting sticker...")}`)
        ...
        if (fs.existsSync(tmpFileOut)){
			...
			console.log(`${chalk.bold.yellow("[+]")} ${chalk.bold.red("Try again...")}`)
		}
    }
